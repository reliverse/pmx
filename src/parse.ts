import type { Agent, Command, ResolvedCommand } from "@reliverse/pm";
...
export const parseNi = ((agent, args, ctx) => {
...
export const parseNr = ((agent, args) => {
  if (args.length === 0) {
    args.push("start");
  }
...
export const parseNu = ((agent, args) => {
  if (args.includes("-i")) {
...
export const parseNun = ((agent, args) => {
  if (args.includes("-g")) {
...
export const parseNlx = ((agent, args) => {
  return getCommand(agent, "execute", args);
}) as Runner;

export const parseNa = ((agent, args) => {
